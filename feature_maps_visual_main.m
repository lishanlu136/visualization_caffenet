...
im = imread('example/images/cat.jpg');
figure;
imshow(im);
...
%convert an image returned by matlab's imread to imread to im_data in caffe's data
%format: W * H * C with BGR channels
im_data = im(:,:,[3,2,1]);   %permute channels from RGB to BGR
im_data = permute(im_data,[2,1,3]); %flip width and height
im_data = single(im_data);   %convert from uint8 to single
im_data = imresize(im_data, [IMAGE_DIM IMAGE_DIM], 'bilinear');    %resize im_data
im_data = im_data - mean_data;   %subtract mean_data (already in W * H * C, BGR)
